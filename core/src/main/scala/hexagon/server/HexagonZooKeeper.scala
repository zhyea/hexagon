...
class HexagonZooKeeper(val config: HexagonConfig) extends Logging {
  ...
  def registerTopicInZkInternal(topic: String): Unit = {
    ...
    info("Begin registering topic {} with {} replicas.", brokerTopicPath, config.replicaNum)
    ZKUtils.createEphemeralPathExpectConflict(zkClient, brokerTopicPath, config.replicaNum.toString)
    info("End registering topic {}.", brokerTopicPath)
  }
